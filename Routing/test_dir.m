...
for i=1:number_of_nodes
    if i==ini.constants.Sender_ID
        node(i)=Node(i,trail{i}(1,1),trail{i}(1,2),'sender');
    elseif ismember(i,ini.constants.Receiver_ID)
        node(i)=Node(i,trail{i}(1,1),trail{i}(1,2),'receiver');
    else
        node(i)=Node(i,trail{i}(1,1),trail{i}(1,2));    
    end
...
%% Main program
for t=0:1:ini.constants.Simulation_time
    X_Coord=zeros(number_of_nodes,1);
    Y_Coord=zeros(number_of_nodes,1);
    for i=1:number_of_nodes
        X_Coord(i)=node(i).x;
    ...
    switch ini.mac.proto
        ...
        case 'directional' %directional cast
            [idxs,mac_delay]=topologydir(number_of_nodes,ini.constants.Sender_ID,X_Coord,Y_Coord,Signal_strength);
        case 'halo'
            [idxs,phantom,mac_delay]=halo(node,halo_cutoff,phantom);   
    end
    
    G=graph(idxs,'upper');
    G.Edges.Weight=EdgeWeights(G,node,Signal_strength,phantom);
...
% allocate time slot in one second
    scheduler=MAC_TDMA(route_table,mac_delay);
    if scheduler.src(1)~=0
        transmit_pkt(scheduler,node,time_slot,t); %transmit packets
    end
% -------------------QoS part end-----------------------------
    % updating nodes
    for i=1:number_of_nodes
...
D=node(ini.constants.Receiver_ID(1)).packets.delay;
fprintf('First receiver throughput is:  %4.2f Mbps\n',T);
fprintf('First receiver delay is: %4.2f second\n\n',D);
